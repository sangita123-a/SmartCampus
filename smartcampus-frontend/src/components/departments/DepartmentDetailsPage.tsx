...
export function DepartmentDetailsPage({ departmentId }: DepartmentDetailsPageProps) {

  const { data, isLoading, isError, error, refetch } = useDepartment(departmentId);



  if (isLoading) return <Loading label="Loading department details..." />;
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Link
            ...
            href={`/dashboard/college-admin/departments/${data.id}/edit`}
      ...
      <section className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5 sm:p-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <span className="text-sm text-[var(--muted)]">
            ...
            {data._count?.courses ?? 0} linked courses
